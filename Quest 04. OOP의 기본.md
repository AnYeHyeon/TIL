...
## Contents 3 : API
: Application programming Interface 의 약자이다. 이는 응용 프로그램들이 서로 상호 작용하기 위해 정의된 인터페이스를 말한다. 즉, API는 하나의 소프트웨어가 다른 소프트웨어와 소통하는 데 사용되는 규약이나 방법른을 의미한다.
- API는 가장 일반적인 형태로 웹 API가 있다. 웹 API는 웹 서비스를 제공하는 서버와 클라이언트 간의 통신을 위한 규칙과 데이터 형식을 제공한다. 웹 API를 통해 클라이언트는 서버의 기능을 호출하거나 데이터를 요청하여 웹 서비스를 이용할 수 있다.

- API를 사용하면 소프트웨어 개발자들은 다른 애플리케이션에서 제공하는 기능과 데이터를 활용할 수 있다. 이를 통해 개발자들은 필요한 기능을 직접 구현하기보다는 이미 구축된 기능을 활용하여 더욱 빠르고 효율적으로 애플리케이션을 개발할 수 있다.
